...
import com.ptu.util.GoodsGrid;
import com.ptu.util.OrdersGrid;
import com.ptu.util.PurseGrid;
import com.ptu.util.UserGrid;
...
public class AdminController {
	...
	/*********************************************************
	 * 用户管理 1.查找所有用户 2.查看用户 3.修改用户 4.删除用户 5.查询用户 lyq
	 * 
	 **********************************************************/
	...
	public String deleteUser(HttpServletRequest request, User user) {
		try {
			userService.deleteUserById(String.valueOf(user.getId()));
		} catch (Exception e) {
	...
	@ResponseBody
	public String deleteGoods(HttpServletRequest request, Goods goods) {
		try {
			goodsService.deleteGoodsByPrimaryKeys(goods.getId());
		} catch (Exception e) {
			return "{\"success\":false,\"msg\":\"删除失败!\"}";
		}
		return "{\"success\":true,\"msg\":\"删除成功!\"}";
	...
	@ResponseBody
	public String deleteOrders(HttpServletRequest request, Orders orders) {
		try {
			ordersService.deleteOrdersByPrimaryKeys(orders.getId());
		} catch (Exception e) {
			return "{\"success\":false,\"msg\":\"删除失败!\"}";
		}
		return "{\"success\":true,\"msg\":\"删除成功!\"}";
